...
```
  which gives us the interval **`r max_avg_steps_interval`**, with `r which.max(avg_steps_interval$avg_steps)` steps
...
print(NA_count)
```
There are in total **`r NA_count`** missing values in the dataset
